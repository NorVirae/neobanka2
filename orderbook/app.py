...
cross_order_books = (
    {}
)  # Cross-chain order books keyed by symbol (same symbols, separate dict)
cross_activity_log = deque(maxlen=1000)
CROSS_ACTIVITY_LOG_PATH = os.getenv(
    "CROSS_ACTIVITY_LOG_PATH", "orderbook_activity_cross.jsonl"
)
order_signatures = {}
...
SUPPORTED_NETWORKS = {
    "hedera": {
        ...
        "tokens": {
            "HBAR": os.getenv(
                "HEDERA_HBAR_TOKEN_ADDRESS",
                os.getenv(
                    "HBAR_TOKEN_ADDRESS", "0x66B8244b08be8F4Cec1A23C5c57A1d7b8A27189D"
                ),
            ),
            "USDT": os.getenv(
                "HEDERA_USDT_TOKEN_ADDRESS",
                os.getenv(
    ...
    "ethereum": {
        ...
        "rpc": os.getenv(
            ...
            os.getenv(
                "WEB3_PROVIDER_SEPOLIA", "https://ethereum-sepolia-rpc.publicnode.com"
            ),
        ),
        "chain_id": int(
            os.getenv(
                "WEB3_CHAIN_ID_ETHEREUM", os.getenv("WEB3_CHAIN_ID_SEPOLIA", "11155111")
            )
        ),
        "contract_address": os.getenv(
            "TRADE_SETTLE_CONTRACT_ADDRESS_ETHEREUM",
            os.getenv(
                "TRADE_SETTLE_CONTRACT_ADDRESS_SEPOLIA",
                "0x10F0F2cb456BEd15655afB22ddd7d0EEE11FdBc9",
            ),
        ),
        "tokens": {
            ...
            "HBAR": os.getenv(
                ...
                os.getenv(
                    "SEPOLIA_HBAR_TOKEN_ADDRESS",
                    "0xb458260166d1456A5ffB46eBbC4270738A515286",
                ),
            ),
            "xZAR": os.getenv(
...
TOKEN_ADDRESSES = {
    "HBAR": os.getenv(
        "HBAR_TOKEN_ADDRESS", SUPPORTED_NETWORKS["hedera"]["tokens"]["HBAR"]
    ),
    "USDT": os.getenv(
        "USDT_TOKEN_ADDRESS", SUPPORTED_NETWORKS["hedera"]["tokens"]["USDT"]
    ),
    "xZAR_ETH": os.getenv(
        "XZAR_ETH_ADDRESS", "0x48f07301e9e29c3c38a80ae8d9ae771f224f1054"
    ),
...
async def get_orderbook_cross(request: Request):
    return await api_service.get_orderbook(
        request=request, order_books=cross_order_books
    )
...
async def check_available_funds(payload: str = Form(...)):
    return api_service.check_available_funds(order_books=order_books, payload=payload)
...
async def faucet(request: Request):
    try:
        ...
        if not token_addr:
            return {
                "status_code": 0,
                "message": f"token not configured for {asset} on {network}",
            }

        client = SettlementClientSame(
            net.get("rpc"), net.get("contract_address"), PRIVATE_KEY
        )
